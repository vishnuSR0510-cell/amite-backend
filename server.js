...
db.connect(err => {
  ...
  db.query(tableQuery, err => {
    ...
    else console.log("✅ Table is ready.");const express = require('express');
const nodemailer = require('nodemailer');
const mysql = require('mysql2');
const cors = require('cors');
require('dotenv').config();
...
// ✅ Create MySQL pool instead of connection
const db = mysql.createPool({
  connectionLimit: 10, // optional
  host: process.env.DB_HOST,
  user: process.env.DB_USER,
...
// ✅ Directly run table creation query without .connect()
const tableQuery = `
  CREATE TABLE IF NOT EXISTS enquiries (
    id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(100),
...
app.post("/send-email", (req, res) => {
  ...
  db.query(sql, values, async (err, result) => {
    ...
    const customerMail = {
      from: process.env.EMAIL_USER,
      to: email || process.env.EMAIL_TO, // fallback
